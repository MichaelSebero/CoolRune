...
# REPO PACKAGES INSTALL
pacman -S --noconfirm --needed lib32-artix-archlinux-support base-devel flatpak kate librewolf python-pip tmux tealdeer jdownloader2 vulkan-icd-loader lib32-vulkan-icd-loader liferea ksnip gnome-keyring kcalc font-manager gwenview gimp gamemode lib32-gamemode fail2ban fail2ban-s6 evince dnscrypt-proxy dnscrypt-proxy-s6 apparmor apparmor-s6 bleachbit blueman unrar bluez-s6 konsole catfish clamav clamav-s6 ark gufw mugshot macchanger networkmanager networkmanager-s6 nm-connection-editor wine-ge-custom wine-mono winetricks ufw-s6 pcmanfm pavucontrol qbittorrent redshift psensor ntfs-3g steam lynis sneedacity element-desktop rkhunter paru proton-ge-custom lib32-mesa vulkan-intel lib32-vulkan-intel protontricks-git appimagelauncher opendoas linux-tkg-pds linux-tkg-pds-headers pamac-nosnap ksysguard tk lightdm-gtk-greeter-settings downgrade portmidi shellcheck npm libreoffice pipewire-pulse pipewire-alsa wireplumber gparted simplescreenrecorder wine-gecko rust python-psutil python-dateutil python-pydub python-xlib python-pyaudio python-pipenv spice-vdagent find-the-command usbguard usbguard-s6 hunspell-en_us gtk-engine-murrine vkbasalt lib32-vkbasalt testdisk yay &&
...
# FIRST COMMANDS AND COOLRUNE IMPORT
pacman-key --init && pacman -Sy --noconfirm --needed p7zip git subversion wget && mkdir /home/coolrune-files && cd /home/$USER/Desktop && eval $coolrune_manual && cd /home/coolrune-files && eval $pacman1 && eval $pacman2 && eval $root_files && eval $nvidia_patch && eval $hardening_script && 7z e coolrune-pacman-1.7z -o/etc -y && pacman -Sy --noconfirm --needed artix-keyring archlinux-keyring artix-archlinux-support && pacman-key --recv-key 3056513887B78AEB --keyserver keyserver.ubuntu.com && pacman-key --lsign-key 3056513887B78AEB && pacman -U --noconfirm 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-keyring.pkg.tar.zst' 'https://cdn-mirror.chaotic.cx/chaotic-aur/chaotic-mirrorlist.pkg.tar.zst' && 7z e coolrune-pacman-2.7z -o/etc -y && pacman-key --populate archlinux artix && pacman -Syyu --noconfirm --needed && eval $dotfiles &&

# REPO PACKAGES REMOVE
pacman -R --noconfirm linux linux-headers pulseaudio pulseaudio-alsa pulseaudio-bluetooth pulseaudio-zeroconf epiphany xfce4-terminal xfce4-screenshooter parole xfce4-taskmanager mousepad leafpad xfburn ristretto xfce4-appfinder atril artix-branding-base artix-grub-theme mpv xfce4-sensors-plugin xfce4-notes-plugin && 

# REPO PACKAGES INSTALL PT 1
pacman -S --noconfirm --needed lib32-artix-archlinux-support base-devel flatpak kate librewolf python-pip tmux tealdeer jdownloader2 vulkan-icd-loader lib32-vulkan-icd-loader liferea ksnip gnome-keyring kcalc font-manager gwenview gimp gamemode lib32-gamemode fail2ban fail2ban-s6 evince dnscrypt-proxy dnscrypt-proxy-s6 apparmor apparmor-s6 bleachbit blueman unrar bluez-s6 konsole catfish clamav clamav-s6 ark gufw mugshot macchanger networkmanager networkmanager-s6 nm-connection-editor wine-ge-custom wine-mono winetricks ufw-s6 pcmanfm pavucontrol qbittorrent redshift psensor ntfs-3g steam lynis sneedacity element-desktop rkhunter paru proton-ge-custom nvidia-utils nvidia-utils-s6 lib32-nvidia-utils nvidia-settings &&

# NVIDIA DKMS INSTALL
pacman -R --noconfirm nvidia && pacman -S --noconfirm --needed nvidia-dkms &&

# REPO PACKAGES INSTALL PT 2
pacman -S --noconfirm --needed protontricks-git appimagelauncher opendoas linux-tkg-pds linux-tkg-pds-headers pamac-nosnap ksysguard tk lightdm-gtk-greeter-settings downgrade portmidi shellcheck npm libreoffice pipewire-pulse pipewire-alsa wireplumber gparted simplescreenrecorder wine-gecko rust python-psutil python-dateutil python-pydub python-xlib python-pyaudio python-pipenv spice-vdagent find-the-command usbguard usbguard-s6 hunspell-en_us gtk-engine-murrine vkbasalt lib32-vkbasalt testdisk yay &&
